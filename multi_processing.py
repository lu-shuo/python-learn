...
def multipool():
    ...
    pool = mp.Pool()
    res = pool.map(pool_job,range(10000))
    print(res)
    st2 = time.time()
...
if __name__=='__main__':
    ...
    v = mp.Value('i', 0)
    lock = mp.Lock()
    p1 = mp.Process(target=multiCoreValue, args=(1,v,lock))
    p2 = mp.Process(target=multiCoreValue, args=(2,v,lock))
    p1.start()
    p2.start()
    p1.join()
    p2.join()
