...
# 要理解多态的好处，我们还需要再编写一个函数，这个函数可以接受一个Animal类型的变量：

def run_twice(classX):
    classX.run()
    classX.run()

# 当我们传入Animal的实例时，run_twice()就打印出Animal is running：
run_twice(Animal())
# 当我们传入Dog的实例时，run_twice()就打印出Dog is running：
run_twice(Dog())
# 当我们传入Cat的实例时，run_twice()就打印出Cat is running：
run_twice(Cat())
...
# 静态语言 vs 动态语言
# 对于函数run_twice(),在动态语言python中,不一定需要传入Animal类型。我们只需要保证传入的对象有一个run()方法就可以了：
class Timer(object):
    def run(self):
        print('Start...')
...
# 而在静态语言（例如Java）来说，如果需要传入Animal类型，则传入的对象必须是Animal类型或者它的子类，否则，将无法调用run()方法。
